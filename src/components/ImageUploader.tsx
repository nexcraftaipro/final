    ...
    for (const file of filesToProcess) {
      const promise = (async () => {
        ...
        if (!isValidImageType(file)) {
          toast.error(`${file.name} is not a valid file format. Only JPEG, PNG, EPS and AI files are supported.`);
          return null;
        }
  ...
  return <div className="bg-gray-800 border border-dashed border-gray-600 rounded-lg overflow-hidden">      
      <div className={`drop-zone flex flex-col items-center justify-center p-10 transition-all duration-300 ${isDragging ? 'border-blue-500 bg-blue-500/10' : 'bg-gray-800/50 hover:bg-gray-800/70'}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
        ...
        <div className="text-center mb-6">
          <p className="text-lg font-medium text-white mb-2">Drag and drop unlimited files here</p>
          <p className="text-sm text-gray-400">
            or click to upload (JPEG/PNG/EPS/AI up to 10GB each)
          </p>
        </div>
        ...
        <input 
          ...
          accept="image/jpeg,image/png,image/jpg,application/postscript,application/illustrator" 
          multiple 
          className="hidden" 
